...
import certifi
import secrets
from datetime import datetime, timedelta  # Mover esta importación aquí arriba
...
# Definición de versión CSS para evitar caché
...
logging.basicConfig(
    filename='/var/www/vhosts/edefrutos2025.xyz/httpdocs/app_debug.log',
    level=logging.DEBUG,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
)

# Handler para imprimir errores también en la consola
...
# Asegura que todas las plantillas tengan acceso a now y css_version
@app.context_processor
def inject_template_vars():
    """Inyecta variables comunes a todas las plantillas"""
    return {
        'now': datetime.now(),
        'css_version': CSS_VERSION if 'CSS_VERSION' in globals() else datetime.now().strftime("%Y%m%d%H%M%S")
    }

@app.route('/static/<path:filename>')
def custom_static(filename):
    """Manejador personalizado para archivos estáticos"""
    try:
        return send_from_directory('static', filename)
    except Exception as e:
        app.logger.error(f"Error al servir archivo estático {filename}: {str(e)}")
        return str(e), 500

@app.route('/static_test')
def static_test():
    """Ruta de diagnóstico para archivos estáticos"""
    ...
    result = {
        ...
        'test_css_exists': os.path.exists(test_css),
        'static_dir_files': os.listdir(static_dir) if os.path.exists(static_dir) else [],
        'css_dir_files': os.listdir(css_dir) if os.path.exists(css_dir) else [],
        'test_css_size': os.path.getsize(test_css) if os.path.exists(test_css) else 0,
        'test_css_permissions': oct(os.stat(test_css).st_mode)[-3:] if os.path.exists(test_css) else None,
    }
    
    return render_template('static_test.html', result=result)
...
@app.route("/welcome")
def welcome():
    # TEMPORALMENTE eliminamos el redireccionamiento para probar los estilos
...
# Añade este código al final de tu app.py
...
def test_styles():
    """Ruta para probar estilos con una plantilla simplificada"""
    return render_template('test_styles.html')

# Asegúrate de que la última línea de tu app.py sea:
